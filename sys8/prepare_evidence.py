...
def _clean_topic_text(desc: str, tags: List[str], noise: Dict[str, List[str]]) -> Tuple[str, bool]:
    desc = desc or ""
    desc = desc.lower()
    desc = URL_RE.sub(" ", desc)
    desc = HANDLE_RE.sub(" ", desc)
    desc = HASHTAG_INLINE_RE.sub(" ", desc)
    tokens = [normalize_token(tok) for tok in TOKEN_RE.findall(desc)]
    ...
    for tag in tags:
        ...
        if not tag_norm:
            continue
        hashtag_tokens.extend(re.split(r"[ _]", tag_norm))
    hashtag_tokens = [
        t for t in hashtag_tokens if t and t not in noise["stopwords"] and t not in noise["platform"]
    ]
    combined = tokens + hashtag_tokens
    insufficient = len(combined) < 5
    return " ".join(combined), insufficient
...
def _build_lex_tokens(desc: str, tags: List[str], noise: Dict[str, List[str]]) -> List[str]:
    desc = desc or ""
    desc = URL_RE.sub(" ", desc.lower())
    desc = HANDLE_RE.sub(" ", desc)
    tokens = [normalize_token(t) for t in TOKEN_RE.findall(desc)]
    tokens = [t for t in tokens if t and t not in noise["stopwords"] and t not in noise["platform"]]
    tag_tokens: List[str] = []
    for tag in tags:
        tag_norm = normalize_token(tag.lower().strip("# "))
...
def prepare_evidence(df: pd.DataFrame, config: Sys8Config) -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        ...
        topic_text, insufficient_topic_evidence = _clean_topic_text(desc, tags_norm, noise_lists)
        ...
        rows.append(
            {
                ...
                "noise_hits": [tok for tok in lex_tokens if tok in noise_lists["platform"]],
